...
EXPORT_SYMBOL(inet_getname);
 ...
 //->__sock_sendmsg_nosec()[by call sock->ops->sendmsg()]->inet_sendmsg()
int inet_sendmsg(struct kiocb *iocb, struct socket *sock, struct msghdr *msg,
		 size_t size)
{
	struct sock *sk = sock->sk;

	sock_rps_record_flow(sk);//rps是一种网络优化技术,可以先不管
	...
	return sk->sk_prot->sendmsg(iocb, sk, msg, size);//对于使用tcp协议来说，调用的是结构体struct proto tcp_prot里面的sendmsg函数，即tcp_sendmsg()，在文件net/ipv4/tcp_ipv4.c
}
...
//涉及到的关键数据结构：ptype_base , inet_protos
