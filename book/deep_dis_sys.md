...
其次，在实现raft的时候，我们使用的是connect的超时参数，但是呢，我们知道raft中一个follower在接收到appendrpc的时候定时器会重新记时，但是在raft_2.c当中用超时参数的方式应该怎么样实现定时器重新记时呢？难道用信号，如果要用信号的话就得用多进程编程吧，而不应该用多线程编程吧?
...
实现原子性的方法：
1. 硬盘原子写（一般支持512字节的原子写）+日志文件（这种一般适用于单机）
2. 原子提交协议（一般为多机）
3. 使用共识算法实现事务原子性（一般多机）
